...
const originalFromObject = proto.WebMessageInfo?.fromObject;
if (proto.WebMessageInfo && !proto.WebMessageInfo.fromObject) {
    proto.WebMessageInfo.fromObject = function(obj) {
        // Return the object as-is instead of trying to convert
        return obj;
    };
}
...
const safeMessageHandler = (originalHandler) => {
    return async (...args) => {
        try {
            ...
            if (error.message.includes('fromObject is not a function')) {
                console.log('⚠️ Protobuf error bypassed:', error.message);
                return; // Skip this message processing
            }
